...
class EarlyStoppingNew():
    """Early stops the training if validation performance doesn't improve after a given patience."""
    def __init__(self, patience=7, verbose=False, delta=0, path='./checkpoint/', trace_func=print, model='checkpoint', filename="pytorch_model.bin"):
        """
        Args:
            patience (int): How long to wait after last time validation loss improved.
                            Default: 7
    ...
    def save_checkpoint(self, val_loss, model, optimizer, scheduler, epoch):
        '''Saves model when validation loss decrease.'''
        if self.verbose:
            if hasattr(self.trace_func, 'info'):
                self.trace_func.info(f'The best score is ({self.val_loss_min:.6f} --> {val_loss:.6f}).  Saving model ...')
            else:
                self.trace_func(f'The best score is ({self.val_loss_min:.6f} --> {val_loss:.6f}).  Saving model ...')
